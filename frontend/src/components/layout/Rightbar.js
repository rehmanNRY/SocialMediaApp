...
"use client"; // necessary for Next.js 14 client components
import { useState } from "react";
import { FiUsers, FiMessageSquare, FiBell, FiSettings, FiTrendingUp } from "react-icons/fi";
import { AiOutlineProfile, AiOutlineTeam, AiOutlineAppstoreAdd } from "react-icons/ai";
import { BsBriefcase, BsGlobe, BsCalendarEvent, BsGear } from "react-icons/bs";
import { MdOutlineAnalytics, MdOutlineHelpCenter } from "react-icons/md";

const Rightbar = () => {
  // State to manage toggles and interactive elements
  const [showMore, setShowMore] = useState(false);

  // Mock data for recent activities and groups
  ...
  return (
    <div className="w-64">
      <div className="w-64 p-4 bg-white shadow-lg flex flex-col gap-4 fixed overflow-y-auto" style={{ height: "calc(100vh - 4rem)" }}>
      {/* Recent Activities */}
      <div className="flex flex-col gap-2.5 border-b py-3">
        <h4 className="text-md font-semibold text-gray-800">Recent Activities</h4>
        {recentActivities.map((activity, index) => (
          <div key={index} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 transition">
            {activity.icon}
            <div>
      ...
      <div className="flex flex-col gap-3 border-t pt-3">
        <h4 className="text-md font-semibold text-gray-800">Suggested Groups</h4>
        {groups.map((group, index) => (
          ...
          >
            ...
            <button className="text-blue-500 text-xs font-semibold">Join</button>
          </div>
        ))}
        {/* Toggle More Section */}
        {showMore && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 transition">
              ...
              <span className="text-gray-700">Discover New Connections</span>
            </div>
            <div className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-100 transition">
